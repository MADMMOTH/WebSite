...
var particleCount = 100,
	...
	linkOpacity = 0.5; // number between 0 & 1
	linkFade = 90, // link fade-out frames
	linkSpeed = 1, // distance a link travels in 1 frame
	glareAngle = -60,
	glareOpacityMultiplier = 0.15,
...
function position(x, y, z) {
	return {
		x: (x * canvas.width) + (((((canvas.width / 2) - mouse.x) * tilt + ((nPos.x - 0.5) * noiseStrength)) * z) * motion),
		y: (y * canvas.height) + (((((canvas.height / 2) - mouse.y) * tilt + ((nPos.y - 0.5) * noiseStrength)) * z) * motion)
	};
}
